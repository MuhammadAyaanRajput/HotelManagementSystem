const Dashboard = (() => {
  ...
  async function load(force = false) {
    ...
    await Promise.all(
      keys.map(async (k) => {
        try {
          results[k] = await Api.list(k, { force });
        } catch (_) {
          results[k] = [];
        }
      }),
    ...
    safeRender(
      () => renderReservationStatusChart(results.reservation),
      'chart-reservation-status',
    );
    safeRender(
      () => renderPaymentMethodChart(results.payments),
      'chart-payment-method',
    );
    safeRender(
      () => renderEmployeeDeptChart(results.employees),
      'chart-employees-dept',
    );
