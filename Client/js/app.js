(function () {
  ...
  function closeMobileNav() {
    navGroupsWrap.classList.remove('is-open');
  }

  function setActiveNav(route) {
    document
      .querySelectorAll(
        '.nav-link[data-route], .nav-dropdown__item[data-route]',
      )
      .forEach((el) => {
    ...
    const images = [
      {
        seed: 'aurelia-lobby',
        title: 'Grand Lobby',
        desc: 'The marble atrium welcomes every guest at check-in.',
      },
      {
        seed: 'aurelia-exterior',
        title: 'Hotel Exterior',
        desc: 'Aurelia\u2019s flagship tower at dusk.',
      },
      {
        seed: 'aurelia-suite',
        title: 'Deluxe Suite',
        desc: 'King bed suites with skyline views.',
      },
      {
        seed: 'aurelia-room2',
        title: 'Executive Room',
        desc: 'Comfortable stays for the modern traveller.',
      },
      {
        seed: 'aurelia-restaurant',
        title: 'Restaurant & Menu',
        desc: 'Seasonal tasting menus from our executive chef.',
      },
      {
        seed: 'aurelia-bar',
        title: 'Rooftop Bar',
        desc: 'Handcrafted cocktails above the skyline.',
      },
      {
        seed: 'aurelia-pool',
        title: 'Infinity Pool',
        desc: 'Open year-round for all registered guests.',
      },
      {
        seed: 'aurelia-spa',
        title: 'Spa & Wellness',
        desc: 'Book treatments directly under Guest Services.',
      },
      {
        seed: 'aurelia-gym',
        title: 'Fitness Studio',
        desc: '24-hour access with personal trainers on call.',
      },
    ];
  ...
  async function runGlobalSearch(query) {
    ...
    guests
      ...
      .forEach((g) =>
        results.push({
          ...
          sub: 'Guest',
          route: 'guests',
          search: `${g.first_name} ${g.last_name}`,
        }),
      );

    rooms
      .filter((r) =>
        `${r.room_number} ${r.type_name}`.toLowerCase().includes(q),
      )
      .slice(0, 4)
      .forEach((r) =>
        results.push({
          ...
          sub: r.type_name,
          route: 'rooms',
          search: r.room_number,
        }),
      );

    reservations
      .filter((r) =>
        `${r.first_name} ${r.last_name} ${r.room_number} ${r.reservation_id}`
          .toLowerCase()
          .includes(q),
      )
      .slice(0, 4)
      .forEach((r) =>
        results.push({
          icon: '📅',
          label: `Reservation #${r.reservation_id}`,
          sub: `${r.first_name} ${r.last_name} · Room ${r.room_number}`,
          route: 'reservation',
          search: String(r.reservation_id),
        }),
      );
    ...
    if (results.length === 0) {
      ...
      `,
        ...
        .join('');
      globalSearchResults
        .querySelectorAll('.search-result[data-route]')
        .forEach((btn) => {
