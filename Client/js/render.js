const UI = (() => {
  ...
  function badgeClass(value) {
    ...
    if (['occupied', 'in progress', 'pending'].includes(v))
  ...
  async function showModule(moduleKey) {
    ...
    document
      ...
      .addEventListener('click', () => openForm(moduleKey, null));
    document
      .getElementById('btn-refresh')
      .addEventListener('click', () => loadAndRender(moduleKey, true));
    document
      .getElementById('module-search')
      .addEventListener('input', (e) =>
        renderTable(filterRows(e.target.value, getFilterValue())),
      );
    if (cfg.filterField) {
      document
  ...
  function populateFilterOptions(cfg) {
    ...
    const values = [
      ...new Set(currentRows.map((r) => r[cfg.filterField]).filter(Boolean)),
    ].sort();
    select.innerHTML =
  ...
  function renderTable(rows) {
    ...
    tbody.querySelectorAll('[data-action="edit"]').forEach((btn) => {
      btn.addEventListener('click', () =>
        openForm(currentModule, findRow(btn.dataset.id)),
      );
    });
    tbody.querySelectorAll('[data-action="delete"]').forEach((btn) => {
      btn.addEventListener('click', () =>
        confirmDelete(currentModule, btn.dataset.id),
      );
    });
  }
  ...
  async function confirmDelete(moduleKey, id) {
    ...
    if (
      !confirm(
        `Delete this ${singular(cfg.label).toLowerCase()} (ID ${id})? This cannot be undone.`,
      )
    )
      return;
    try {
  ...
  async function openForm(moduleKey, existingRow) {
    ...
    formEl.innerHTML = cfg.fields
      .map((field) => buildField(field, existingRow, refData))
      .join('');
